...
class ModelsValUsuario{
    ...
    public function valUsuario($usuario, $editarUnico = null, $id = null)
    {
        ...
        if(!empty($this->editarUnico) AND ($this->editarUnico == true)){
            $valUsuario->exeReadEspecifico("SELECT idUsuario FROM usuarios WHERE nome =:nome AND idUsuario <>:id LIMIT :limit", "nome={$this->usuario}&limit=1&id={$this->id}");
        }else{
            $valUsuario->exeReadEspecifico("SELECT idUsuario FROM usuarios WHERE nome =:nome LIMIT :limit", "nome={$this->usuario}&limit=1");
        }        
