    ...
    private function updateEditSenha(){
        ...
        if ($this->result) {
            ...
            if ($upAtualSenha->getResult()) {
                $_SESSION['msg'] = "<div id='message_div' class='alert alert-success'>Senha atualizada com sucesso!</div>";
                $this->result = true;
            } else {
                $_SESSION['msg'] = "<div id='message_div' class='alert alert-danger'>Erro: A senha não foi atualizada!</div>";
                $this->result = false;
            }
        } else {
            $_SESSION['msg'] = "<div id='message_div' class='alert alert-danger'>Erro: A senha não foi atualizada!</div>";
